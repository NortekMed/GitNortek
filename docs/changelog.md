...
* Show a dual-column diff overview bar in complete-file views, with left and right change markers using the editor's addition and deletion colors, alongside previous/next modified-block controls
...
* Add stash access to the uncommitted-changes context menu and a confirmation-backed discard-all control for working-tree changes
...
* Base modified-block navigation on each block's first line so scrolling away from a block lets Up return to that block and Down advance to the next one
...
* Keep large commit histories responsive with adaptive ahead-of-viewport loading and show eight graph lanes by default; clip additional lanes within the user-resizable graph column
* Prevent duplicate or reentrant repository tab updates when opening submodules
* Remove repository tabs immediately while their background cleanup continues, and add bulk tab-close context-menu actions
...
* Restore the open tabs from the previously active window at startup, with an option to restore only the selected repository
...
* Use a compact 8-point font throughout repository details, editors, navigation rows, and menus
