...
| --- | --- | --- | --- | --- |
| 2026-08-12 | 7 | Manual validation required | Matched navigator section typography to the existing Open/Recent headers by using the same bold small-caps treatment and bright header foreground. | Recheck section typography in the active theme, then continue collecting visual feedback. |
| 2026-08-12 | 7 | Manual validation required | Added palette-aware compact section/row rendering, uppercase section labels, counts, dividers, disabled placeholder text, current-branch marker, ahead/behind text, elision, keyboard focus, a 280-pixel default width, clamped persisted width, and provisional chooser/navigator proportions. `gitnortek` and `test_repository_sidebar` build; focused MainWindow, repository sidebar, reference list, and Submodule tests pass. | Manually inspect light/dark themes, scaling, section proportions, disclosure indicators, selected/current states, tracking labels, long names, scrolling, and real interactions before visual tuning or old-selector removal. |
